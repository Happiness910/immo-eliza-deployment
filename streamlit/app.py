...
import requests
import time 
from cities import cities_by_province
from cities import PROVINCE_CITIES, CITY_TO_PROVINCE
...
with placeholder.container():
    ...
    st.markdown(
        ...
        <h1 style='color:#8B1E3F;'>
        Welcome to Immo Eliza
        </h1>

        <h3>
        AI-powered Belgian House Price Estimator
        </h3>
...
with col2:
    ...
    # province = st.selectbox(
    #     "Province",
    #     sorted(cities_by_province.keys())
    # )

    # city = st.selectbox(
    #     "City",
    #     sorted(cities_by_province[province])
    # )

    province = st.selectbox(
        "Province",
        sorted(PROVINCE_CITIES.keys())
    )
    ...
    if CITY_TO_PROVINCE[city] != province:

        st.error(
            "❌ The selected city does not belong to this province."
        )
...
if st.button("💰 Predict Price", use_container_width=True):
    ...
    Based on the average prediction error of the model (MAE ≈ €{MAE:,.0f}), the estimated market value is likely between:

    ### **€ {lower:,.0f}  —  € {upper:,.0f}**

    This interval reflects the typical uncertainty observed on unseen properties.
    """
        )
    ...
    st.write(
        """
This estimate is generated by an **XGBoost machine learning model**
trained on Belgian residential property transactions.

**Model performance**

- R² score: **0.66**
- Mean Absolute Error (MAE): **≈ €100,000**

The prediction should therefore be interpreted as an **estimated market value** rather than an exact selling price.
"""
    )
    ...
    st.subheader("💡 Factors not captured by the model")

    st.write(
        """
The model relies on a limited set of publicly available characteristics.

Actual selling prices may differ because of factors such as:
...
This prediction is intended for informational purposes only and should not be considered an official property valuation.

For investment decisions or property sales, a professional appraisal is recommended.
"""
    )
...
st.caption(
    "❤️ Built with Streamlit, FastAPI, Docker and XGBoost. By Happiness 🌞"
)